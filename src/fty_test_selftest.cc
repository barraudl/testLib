/*  =========================================================================
    fty_test_selftest.c - run selftests
...
// Tests for stable public classes:
    { "fty_test", fty_test_test },
#ifdef FTY_TEST_BUILD_DRAFT_API
    { "private_classes", fty_test_private_selftest },
...
{
    test_item_t *item;
    printf ("Running fty-test selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_test_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-test test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
